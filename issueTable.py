        ...
        def isb():
            if (len(c.get())) == 0:
             ...
             try:
                    ...
                    self.mycursor.execute("Select availability from book where availability = 'YES' and book_id = %s and (reserved_stud_id = %s or reserved_stud_id = 0)", [c.get(),d.get()])
                    ...
                    try:
                     if self.pc:
                        ...
                        self.mycursor.execute("Insert into issue_book(book_id,stud_id,issue_date,return_date) values (%s,%s,%s,%s)",
                                              [book, stud, idate,''])
                        self.conn.commit()
                        self.mycursor.execute("Update book set availability = 'NO', reserved = 'NO' , reserved_stud_id = 0 where book_id = %s", [book])
                        self.conn.commit()
